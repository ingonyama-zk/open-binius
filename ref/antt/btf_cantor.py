...
class btf():
    ...
    def _mul_abstract(a_hi, a_lo, a_sum, b_hi, b_lo, b_sum):
        # mod: x_i^2 = x_i + (x_i >> 1)
        # (a_hi * x_i + a_lo)(b_hi * x_i + b_lo) = (a_lo * b_lo) + ((a_hi * b_hi) * (x_i >> 1))
        #                                          [ (a_hi * b_lo + a_lo * b_hi) + (a_hi * b_hi) ] * x_i +
        # note that since x_i = v_{2^i}, the (2^i)'th basis element => (x_i >> 1) = v_{2^i-1}
        lo = a_lo * b_lo
        hi = a_sum * b_sum + lo
        mx = a_hi * b_hi
        mx = mx * btf(1 << (mx.nbits - 1), nlvls=mx.nlvls)
        lo += mx
        # concatenate
